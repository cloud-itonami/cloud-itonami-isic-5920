(ns musicops.governor-contract-test
  ...
  (testing "clean production-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-production-record :catalog-id "catalog-1" :patch {:session-date "2026-07-10" :take-count 12}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-rights-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-rights-concern :catalog-id "catalog-1"
                                :patch {:concern "possible uncleared sample" :confidence 0.99}}
                               ctx)]
      (is (some? result))
...
(deftest unregistered-catalog-entry-hard-hold
  (testing "unregistered catalog entry -> permanent HARD hold, never escalates"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-3" :phase 3}]
      (exec-request actor "t3"
                     {:op :log-production-record :catalog-id "unknown-catalog"
                      :patch {:session-date "2026-07-01"}}
                     ctx)
      (is (= 0 (count (store/coordination-log db)))
          "HARD hold must never commit"))))

(deftest unverified-catalog-entry-hard-hold
  (testing "registered but unverified catalog entry -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4" :phase 3}
          result (exec-request actor "t4"
                               {:op :log-production-record :catalog-id "catalog-3"
                                :patch {:session-date "2026-07-01"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unverified catalog entry must HARD hold"))))
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-production-record :catalog-id "catalog-1"
                                :patch {:session-date "2026-07-01"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into rights-license-finalization/royalty-payment-determination scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-production-record :catalog-id "catalog-1" :patch {:take-count 3}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-production-record :catalog-id "unknown" :patch {:take-count 3}}
                     ctx)
...
(deftest rights-royalty-decision-is-never-auto-commit-eligible
  (testing "CRITICAL Wave 4 person-facing-service guardrail: a proposal that directly finalizes a rights-licensing grant or a royalty-payment determination must never reach :commit at any phase, and flag-rights-concern must never be a member of any phase's :auto set"
    (let [db (store/seed-db)
          poisoned-advisor (reify advisor/Advisor
                              (-advise [_ _ req]
                                (assoc (advisor/infer nil req)
                                       :rationale "finalize the rights license and authorize the royalty payment")))
          actor (op/build db {:advisor poisoned-advisor})
          ctx {:actor-id "test-9" :phase 3}
          result (exec-request actor "t9"
                               {:op :log-production-record :catalog-id "catalog-1" :patch {:take-count 1}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "a rights-licensing/royalty-payment-finalizing proposal must never commit"))))
