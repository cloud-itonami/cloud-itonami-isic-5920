(ns musicops.phase
  "Phase 0->3 staged rollout for the ISIC-5920 sound-recording/music-
  publishing operations-coordination actor.
    ...
    Phase 1  assisted-logging       -- production-record logging
                                        ...
                                        human approval.
    Phase 2  assisted-coordination  -- adds production scheduling and
                                        release coordination, still
                                        approval.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                        `:log-production-record`/
                                        `:schedule-production-operation`/
                                        `:coordinate-release` may
                                        auto-commit.
                                        `:flag-rights-concern` NEVER
                                        auto-commits, at any phase.

  `:flag-rights-concern` is deliberately ABSENT from every phase's
  `:auto` set, including phase 3 -- a permanent structural fact, not a
  rollout milestone still to come. Flagging a rights concern always
  needs a human to actually look at it. `musicops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [musicops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-rights-concern` is a member of
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-production-record :schedule-production-operation
                                               :coordinate-release}                                             :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-production-record :schedule-production-operation :coordinate-release}}})
...
(defn gate
  ...
  - `:flag-rights-concern` is never auto-eligible at any phase, so it
...
(defn verdict->disposition
  "Map a MusicOpsGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
