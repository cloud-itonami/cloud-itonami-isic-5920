(ns musicops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (MusicOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MusicOpsGovernor (:govern)
  ...
  human label/studio operations coordinator. The approver resumes with
...
(defn build
  ...
  `musicops.store/Store`).
  opts:
    :advisor      -- a `musicops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MusicOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MusicOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :catalog-id (:catalog-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :catalog-id (:catalog-id request) :by (:by approval)}]}
            {:disposition :hold
